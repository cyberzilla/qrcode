...
/**
 * QRCode.php — Modern PHP 8.1+ QR Code Generator
 *
 * A modernized, single-file QR code generator with fluent API,
 * typed properties, enums, and no backward compatibility baggage.
 *
 * Usage:
 *   $qr = new QRCode('Hello World', ErrorCorrection::Medium);
 *   $qr->size(600)
...
class QRCode
{
    private const MODE_NUMBER    = 1;
    private const MODE_ALPHA_NUM = 2;
    private const MODE_8BIT_BYTE = 4;
    private const MODE_KANJI     = 8;

    private const PATTERN000 = 0;
    private const PATTERN001 = 1;
    private const PATTERN010 = 2;
    private const PATTERN011 = 3;
    private const PATTERN100 = 4;
    private const PATTERN101 = 5;
    private const PATTERN110 = 6;
    private const PATTERN111 = 7;

    private const EC_L = 1;
    private const EC_M = 0;
    private const EC_Q = 3;
    private const EC_H = 2;
    ...
    private string $detectedMode = 'Byte';
    private int $bestMaskPattern = 0;

    private float $moduleRadius = 0.0;
    private ModuleShape $moduleShape = ModuleShape::Square;
    private ?array $finderStyle = null;

    private int $renderSize = 400;
    private string $renderFg = '#000000';
    private string $renderBg = '#ffffff';
    private int $renderQuality = 85;
    private bool $renderScalable = false;
    private ?string $renderTitle = null;
    private ?string $renderDesc = null;
    private int $renderMargin = 2;
    private ?string $logoPath = null;
    private array $logoOptions = ['ratio' => 0.2, 'padding' => 6, 'radius' => 15];
    ...
    public function __construct(
        ...
        $this->ecLevelChar = $ec->value;
        $this->quiet = max(0, $quietZone);
        $this->text = $data;
        $this->errorCorrectionLevel = $ec->internal();
        ...
        if (!$success) {
            throw new \RuntimeException(
                "Data too long for QR versions {$minVer}-{$maxVer} with EC level {$ec->value}."
            );
        }
    }
    ...
    public function colors(string $foreground, string $background = '#ffffff'): static
    {
    ...
    public function moduleRadius(float $ratio): static
    {
        $this->moduleRadius = max(0.0, min(0.5, $ratio));
        return $this;
    }

    public function moduleShape(ModuleShape $shape): static
    {
        $this->moduleShape = $shape;
        return $this;
    }

    public function finderStyle(
        ?string $outerColor = null,
        ?string $innerColor = null,
        ?float $outerRadius = null,
    ...
    public function scalable(bool $scalable = true): static
    {
        $this->renderScalable = $scalable;
        return $this;
    }

    public function accessibility(?string $title, ?string $desc = null): static
    {
    ...
    public function render(
        ...
        return match ($format) {
            OutputFormat::PNG => $this->logoPath !== null
                ? $this->toPNGWithLogo(
                ...
                : ($this->labelText !== null
                    ...
                    : $this->toPNG($filename, $size, $fg, $bg)
                ),

            OutputFormat::SVG => $this->renderSVG($filename, $size, $fg, $bg),

            OutputFormat::WEBP => $this->toWEBP($filename, $size, $fg, $bg, $this->renderQuality),

            OutputFormat::GIF => $this->renderGif($filename, $size),

            OutputFormat::HTML => $this->renderAndSave($this->toHTML($size, $fg, $bg), $filename),

            OutputFormat::ASCII => $this->toASCII($this->renderMargin),

            OutputFormat::DataURI => 'data:image/png;base64,' . base64_encode($this->toPNG(null, $size, $fg, $bg)),

            OutputFormat::Base64 => base64_encode($this->toPNG(null, $size, $fg, $bg)),

            OutputFormat::ImgTag => '<img src="' . $this->renderGifDataUri($size) . '"'
    ...
    private function renderSVG(?string $filename, int $size, string $fg, string $bg): string|bool
    {
        $svg = match (true) {
            $this->logoPath !== null => $this->toSVGWithLogo(
                $this->logoPath, $size,
                $this->logoOptions['ratio'] ?? 0.2, $fg, $bg,
                $this->logoOptions['padding'] ?? 6,
            ...
            $this->labelText !== null => $this->toSVGWithLabel(
                $this->labelText, $size,
                $this->labelOptions['size'] ?? 0.1,
                $this->labelOptions['color'] ?? '#000000', $fg, $bg,
            ...
            default => $this->toSVG($size, $fg, $bg, $this->renderScalable, $this->renderTitle, $this->renderDesc),
        };
    ...
    private function renderGif(?string $filename, int $size): string|bool
    {
    ...
    private function renderAndSave(string $data, ?string $filename): string|bool
    {
        if ($filename !== null) {
            return (bool) file_put_contents($filename, $data);
        }
    ...
    private function drawModulesPNG($img, int $total, int $moduleSize, int $offset, int $fgColor, int $bgColor): void
    {
        ...
        if ($shape === ModuleShape::Square && $this->moduleRadius <= 0 && !$hasFinderStyle) {
            for ($row = 0; $row < $total; $row++) {
                for ($col = 0; $col < $total; $col++) {
        ...
        if ($shape === ModuleShape::Dot || $shape === ModuleShape::Diamond) {
            $dotScale = 0.80;
    ...
    private function toPNG(?string $filename = null, int $size = 200, string $foreground = '#000000', string $background = '#ffffff')
    {
        ...
        $needsSmooth = $this->moduleShape !== ModuleShape::Square;
    ...
    private function toPNGWithLogo(
        string $logoPath,
        ?string $filename = null,
        int $size = 200,
        float $logoRatio = 0.2,
    ...
    private function toPNGWithLabel(
        string $label,
        ?string $filename = null,
        int $size = 200,
        float $labelSize = 0.1,
    ...
    private function toSVGWithLogo(
        string $logoPath,
        int $size = 200,
        float $logoRatio = 0.2,
    ...
    private function toSVGWithLabel(
        string $label,
        int $size = 200,
        float $labelSize = 0.1,
    ...
    private function toSVG(int $size = 200, string $foreground = 'black', string $background = 'white', bool $scalable = false, ?string $title = null, ?string $alt = null): string
    {
        $total = $this->getModuleCount();
        $moduleSize = $size / $total;
    ...
    private function toHTML(int $size = 200, string $foreground = '#000000', string $background = '#ffffff'): string
    {
        $total = $this->getModuleCount();
        $moduleSize = (int)floor($size / $total);
    ...
    private function renderGifDataUri(int $size = 200): string
    {
        $total = $this->getModuleCount();
        $moduleSize = (int)floor($size / $total);
        $offset = (int)floor(($size - $moduleSize * $total) / 2);
    ...
    private function toWEBP(?string $filename = null, int $size = 200, string $foreground = '#000000', string $background = '#ffffff', int $quality = 90)
    {
        if (!extension_loaded('gd')) {
    ...
    private function toASCII(int $margin = 2): string
    {
        $total = $this->getModuleCount();
        $size = $total + $margin * 2;
        $min = $margin;
